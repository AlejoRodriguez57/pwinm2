...
import { eliminarUbicaciones } from "@/adapters/catalogos";


export default function FormularioDeleteUbicacion() {
    ...
    async function manejarEliminar(e) {
        ...
        const idUbicacion = Number(id);

        if (!idUbicacion) {

            setError(
                "Ingresá un ID de ubicacion válido."
            );
        ...
        const confirmar = window.confirm(
            `¿Seguro que querés eliminar la ubicacion ${idUbicacion}?`
        );
        ...
        try {
            ...
            await eliminarUbicaciones(idUbicacion);

            setMensaje(
                `La ubicacion ${idUbicacion} fue eliminada correctamente.`
            );
